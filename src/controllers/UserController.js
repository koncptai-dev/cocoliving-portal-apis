  ...
  try {
    ...
    if (!phone) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to send phone OTP - phone number required",
        "user",
        userId
      );
    ...
    if (!/^\d{10}$/.test(phone)) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to send phone OTP - invalid phone format",
        "user",
        userId
      );
    ...
    if (!user) {
      await logApiCall(
        ...
        res,
        404,
        "Failed to send phone OTP - user not found",
        "user",
        userId
      );
    ...
    await logApiCall(
      ...
      res,
      200,
      "Sent phone OTP for verification",
      "user",
      userId
    );
    ...
    await logApiCall(
      ...
      res,
      500,
      "Error occurred while sending phone OTP",
      "user",
      req.user?.id || 0
    );
  ...
  try {
    ...
    if (!phone || !otp) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to verify phone OTP - phone and OTP required",
        "user",
        userId
      );
    ...
    if (record.otp !== otp) {
      ...
      await logApiCall(
        ...
        res,
        400,
        "Failed to verify phone OTP - incorrect OTP",
        "user",
        userId
      );
    ...
    await logApiCall(
      ...
      res,
      500,
      "Error occurred while verifying phone OTP",
      "user",
      userId
    );
  ...
  try {
    ...
    if (!email) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to send email OTP - email required",
        "user"
      );
      return res
        .status(400)
    ...
    if (!/^\S+@\S+\.\S+$/.test(email)) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to send email OTP - invalid email format",
        "user"
      );
      return res
        .status(400)
    ...
    if (existingUser && !existingUser.registrationToken) {
      await logApiCall(
        ...
        res,
        401,
        "Failed to send email OTP - user already registered",
        "user"
      );
      return res
        .status(401)
...
exports.registerUser = async (req, res) => {
  try {
    const { fullName, email, phone, userType, gender, dateOfBirth, otp } =
      req.body;
    ...
    if (!email || !otp) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to register user - email and OTP required",
        "user"
      );
    ...
    if (!otpRecord) {
      await logApiCall(
        ...
        res,
        400,
        "Failed to register user - OTP not found or expired",
        "user"
      );
    ...
    if (otpRecord.attempts >= 5) {
      ...
      await logApiCall(
        ...
        res,
        429,
        "Failed to register user - too many wrong OTP attempts",
        "user"
      );
    ...
    if (otpRecord.expiresAt < new Date()) {
      ...
      await logApiCall(
        ...
        res,
        400,
        "Failed to register user - OTP expired",
        "user"
      );
      return res
        .status(400)
    ...
    if (otpRecord.otp !== otp) {
      ...
      await logApiCall(
        ...
        res,
        400,
        "Failed to register user - incorrect OTP",
        "user"
      );
      return res.status(400).json({ message: "Incorrect OTP" });
    }
  ...
  try {
    ...
    if (!user) {
      await logApiCall(
        ...
        res,
        404,
        `Updated user profile - user not found (ID: ${id})`,
        "user",
        parseInt(id)
      );
      return res.status(404).json({ message: "User not found" });
  ...
  try {
    ...
    if (!user) {
      await logApiCall(
        ...
        res,
        404,
        `Deleted user account - user not found (ID: ${userId})`,
        "user",
        parseInt(userId)
      );
      return res.status(404).json({ message: "user not found" });
    ...
    if (user.status === 0) {
      await logApiCall(
        ...
        res,
        400,
        `Deleted user account - user already deleted (ID: ${userId})`,
        "user",
        parseInt(userId)
      );
  ...
  try {
    ...
    if (!user || user.status === 0) {
      await logApiCall(
        ...
        res,
        404,
        `Viewed user details - user not found (ID: ${id})`,
        "user",
        parseInt(id)
      );
    ...
    await logApiCall(
      ...
      res,
      200,
      `Viewed user details: ${user.fullName} (ID: ${id})`,
      "user",
      parseInt(id)
    );
