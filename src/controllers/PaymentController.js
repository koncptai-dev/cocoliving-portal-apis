...
const User = require('../models/user');
const Booking = require('../models/bookRoom');
const PropertyRateCard = require('../models/propertyRateCard'); // Added for property name in email
const Property = require('../models/property'); // Added for include in rateCard query
const { getOrderStatus } = require('../utils/phonepe/phonepeApi');
const phonepeConfig = require('../utils/phonepe/phonepeConfig');
const sequelize = require('../config/database');
const { Op } = require('sequelize');
...
const { bookingConfirmationEmail } = require('../utils/emailTemplates/emailTemplates');
    ...
    if (isSuccessEvent(eventType, state)) {
      await sequelize.transaction(async (t) => {
        ...
        if (tx.pendingBookingData) {
          ...
          // NEW: SEND BOOKING CONFIRMATION EMAIL AFTER SUCCESSFUL PAYMENT & BOOKING CREATION
          try {
            ...
            if (user && rateCard && rateCard.property) {
              const emailTemplate = bookingConfirmationEmail({
                userName: user.fullName,
                bookingId: booking.id,
                propertyName: rateCard.property.name,
              ...
              await sendEmail({
                to: user.email,
                subject: 'Your Coco Living Booking is Confirmed! 🎉',
                html: emailTemplate.html,
                attachments: emailTemplate.attachments,
              });

              console.log(`Booking confirmation email sent to ${user.email} for Booking ID: ${booking.id}`);
            }
          } catch (emailErr) {
            console.error('Failed to send booking confirmation email:', emailErr.message);
            // Email failure should NOT break the payment/booking process
          }
        }
      });
