...
const User = require('../models/user');
const PropertyRateCard = require("../models/propertyRateCard");
const Property = require('../models/property'); // Added for property name in email
const { logActivity } = require("../helpers/activityLogger");
const { Op } = require('sequelize');
const moment = require('moment');
const { logApiCall } = require("../helpers/auditLog");
...
const { bookingCancelledByUserEmail } = require('../utils/emailTemplates/emailTemplates');

// create booking for user (deprecated - now handled via payment webhook)
exports.createBooking = async (req, res) => {
  try {
    const { rateCardId, checkInDate, duration } = req.body;
    const userId = req.user.id;
        ...
        [Op.or]: [
          {
            ...
            checkInDate: { [Op.lte]: checkOutDateFormatted }
          },
          {
            checkOutDate: { [Op.gte]: checkInDateFormatted },
            checkInDate: { [Op.lte]: checkOutDateFormatted }
...
exports.cancelBooking = async (req, res) => {
  try {
    ...
    // Find booking with necessary includes for email
    const booking = await Booking.findOne({
      where: { id: bookingId, userId },
      include: [
        { model: Rooms, as: "room" },
    ...
    try {
      ...
      if (user && booking.rateCard && booking.rateCard.property) {
        const emailTemplate = bookingCancelledByUserEmail({
          userName: user.fullName,
          bookingId: booking.id,
          propertyName: booking.rateCard.property.name,
        ...
        console.log(`Cancellation email sent to ${user.email} for Booking ID: ${booking.id}`);
      }
    } catch (emailErr) {
      console.error('Failed to send user cancellation email:', emailErr.message);
      // Do not break cancellation if email fails
    }
